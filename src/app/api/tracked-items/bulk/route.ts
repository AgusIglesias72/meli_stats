...
import { stores, storeUsers, trackedItemsConfig, trackedItemsData } from '@/lib/db/schema';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingItems = await db.select({ item_id: trackedItemsConfig.item_id })
    ...
    const newItemIds = itemIds.filter((id: string) => !existingItemsMap.has(id));
    ...
    const itemsToInsert = newItemIds.map((itemId: string) => ({
      user_id: authUserId,
      ...
      processing_message: null as string | null
    }));

    // Insertar todos los nuevos items de una sola vez - solo la configuración
    const insertedItems = await db.insert(trackedItemsConfig)
      .values(itemsToInsert)
      .returning({ id: trackedItemsConfig.id, item_id: trackedItemsConfig.item_id });
    ...
    if (insertedItems && insertedItems.length > 0) {
      ...
      processItemsInBackground(insertedItems.map(i => ({ id: i.id, item_id: i.item_id! })), storeData)
...
async function processItemsInBackground(
  ...
  for (let startIdx = 0; startIdx < items.length; startIdx += batchSize * concurrencyLimit) {
    ...
    for (let i = 0; i < concurrencyLimit && startIdx + i * batchSize < items.length; i++) {
      ...
      batchPromises.push(processBatch(batch, storeData));
    }
...
async function processBatch(
  ...
  const results = await Promise.allSettled(
    batch.map(item => processItem(item, storeData))
  );
    ...
    try {
      ...
      if (!itemResponse.ok) {
        ...
        await db.update(trackedItemsConfig)
          ...
          .where(eq(trackedItemsConfig.id, item.id));
