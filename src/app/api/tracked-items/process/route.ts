...
import { stores, trackedItemsConfig, trackedItemsData } from '@/lib/db/schema';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const conditions = [eq(trackedItemsConfig.processing_status, 'pending')];

    // Filtrar por tienda si se especifica
    if (store_id) {
      conditions.push(eq(trackedItemsConfig.store_id, store_id));
    }
    ...
    const pendingItems = await db.select({
      ...
      .orderBy(asc(trackedItemsConfig.id))
      .limit(batch_size);
    ...
    for (const storeId of Object.keys(itemsByStore)) {
      ...
      const storeResults = await processStoreItems(itemsByStore[storeId], storeData);
