...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`📊 [EXPORT-PRODUCTS] Found ${activeStores.length} active stores to process`);
    ...
    for (let storeIndex = 0; storeIndex < activeStores.length; storeIndex += maxConcurrentStores) {
      const concurrentStores = activeStores.slice(storeIndex, storeIndex + maxConcurrentStores);
      ...
      const storePromises = concurrentStores.map(async (store, idx) => {
        ...
        try {
          console.log(`\n🏪 [STORE ${currentStoreIndex + 1}/${activeStores.length}] Starting: ${store.name || store.store_id}`);
        ...
        while (hasMore) {
          const pageItems = await db.select({
            ...
            .where(eq(items.seller_id, String(store.ml_user_id)))
          ...
          if (pageItems && pageItems.length > 0) {
            ...
            console.log(`    - Page ${page + 1}: ${pageItems.length} items (total so far: ${allItems.length})`);
            page++;
      ...
      if (storeIndex + maxConcurrentStores < activeStores.length) {
    ...
    console.log(`  - Total stores processed: ${activeStores.length}`);
