...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`📊 [FIX-ORDERS-SHIPPING] Found ${stores.length} active stores`);
    ...
    for (const store of stores) {
      ...
      const { data: orders, error: ordersError } = await supabase
        .from('orders')
        .select('id, shipping_id, shipping_amount, charge_shipping, net_received_amount')
        .eq('store_id', store.ml_user_id)
        .gte('date_created', '2025-01-01T00:00:00.000Z')
        .not('shipping_id', 'is', null)
        .order('date_created', { ascending: false });

      if (ordersError) {
        console.error(`❌ Error fetching orders for store ${store.store_id}:`, ordersError);
        totalErrors++;
        continue;
      }
      ...
      for (let i = 0; i < orders.length; i += batchSize) {
        ...
        const batchPromises = batch.map(async (order) => {
          try {
            ...
            const needsUpdate = 
              order.shipping_amount !== correctShippingAmount || 
              order.charge_shipping !== correctChargeShipping;

            if (needsUpdate) {
              const { error: updateError } = await supabase
                .from('orders')
                .update({
                  shipping_amount: correctShippingAmount,
                  charge_shipping: correctChargeShipping,
